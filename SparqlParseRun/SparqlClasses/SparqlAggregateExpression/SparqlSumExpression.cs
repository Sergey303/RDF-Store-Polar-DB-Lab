    ...
    class SparqlSumExpression : SparqlAggregateExpression
    {
        public SparqlSumExpression() :base()
        {
            ...
            TypedOperator = result =>
            {
                ...
                var firsts = Expression.TypedOperator(@group[0]);
                if (group.Length == 1)
                    return firsts;
                return firsts.Change(f=>f + @group.Skip(1).Sum(sparqlResult => (double)Expression.TypedOperator(sparqlResult).Content));
            };
        }
    }
}
